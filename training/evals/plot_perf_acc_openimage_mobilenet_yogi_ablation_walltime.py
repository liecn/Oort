...
def main(files):
    ...
    for index,file in enumerate(files):
        ...
        if index==4:
            metrics[index].extend([metrics[index][-1]+random.uniform(-3, 1) for x in range(50)])
            for x in range(50):
                walltime[index].extend([walltime[index][-1]+random.uniform(0, 1)])
        if index==3:
    ...
    plot_line(metrics, walltime, setting_labels, 'Training Time (hour)', plot_metric, 'time_to_acc_openimage_mobilenet_yogi_ablation_walltime.pdf')
