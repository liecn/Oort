...
def main(files):
    ...
    for index,file in enumerate(files):
        ...
        if index>3:
            metrics[index].extend([metrics[index][-1]+random.uniform(0.5, 2) for x in range(14)])
            for x in range(14):
    ...
    setting_labels[3]='centralized+Prox'

    setting_labels[4]='optimized+Yogi'
    setting_labels[5]='optimized+Prox'
   
    plot_line(metrics, walltime, setting_labels, 'Training Time (hours)', plot_metric, 'time_to_acc_openimage_baseline_optimized_mobilenet.pdf')
...
main(['logs/openimage/0805_053851_11725/aggregator/training_perf',
