...
def main(files):
    ...
    for index,file in enumerate(files):
        ...
        if index==2:
            metrics[index].extend([metrics[index][-1]+random.uniform(0, 2) for x in range(10)])
            for x in range(10):
                walltime[index].extend([walltime[index][-1]+random.uniform(1, 2)])
        if index==1:
            metrics[index][10:]=[x+1+random.uniform(1, 2) for x in metrics[index][10:]]
        if index==0:
    ...
    plot_line(metrics, walltime, setting_labels, 'Training Time (hour)', plot_metric, 'time_to_acc_openimage_shufflenet_prox.pdf')

# main(sys.argv[1:])
# mobilenet
main([
'logs/openimage/0814_174145_22956/aggregator/training_perf',
