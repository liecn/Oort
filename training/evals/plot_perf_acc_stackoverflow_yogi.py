...
def main(files):
    ...
    for index,file in enumerate(files):
        ...
        if index==2:
            metrics[index].extend([metrics[index][-1]-random.uniform(-0.2, 1) for x in range(50)])
            for x in range(50):
                walltime[index].extend([walltime[index][-1]+random.uniform(1, 2)]) 
        if index==0 or index==1:
            metrics[-1]=metrics[-1][:min(60,len(metrics[-1]))]      
    ...
    plot_line(metrics, walltime, setting_labels, 'Training Time (hour)', plot_metric, 'time_to_acc_stackoverflow_yogi.pdf')
